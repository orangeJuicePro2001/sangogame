...
class KeyboardControl {
    ...
    private static tryTriggerSceneObjectClickEvent(): void {
        var d: number = Game.player.sceneObject.avatarOri;
    ...
    private static update(): void {
        ...
        var xGrid: number = Math.floor(Game.player.sceneObject.x / Config.SCENE_GRID_SIZE) + this.dirOffsetArr[this.dir][0];
        var yGrid: number = Math.floor(Game.player.sceneObject.y / Config.SCENE_GRID_SIZE) + this.dirOffsetArr[this.dir][1];
        if (ClientWorld.data.moveToGridCenter) {
            toX = xGrid;
            toY = yGrid;
        }
    ...
    static moveDirectGrid(xGrid: number, yGrid: number): void {
        var scene = Game.currentScene as ProjectClientScene;
        // 面向目标
        var oldDir: number = Game.player.sceneObject.avatarOri;

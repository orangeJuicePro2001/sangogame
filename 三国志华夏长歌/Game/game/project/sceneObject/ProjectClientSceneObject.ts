...
class ProjectClientSceneObject extends SceneObjectCommon {
     ...
     * @return 是否触发过事件
     */
    protected touchEventHandle(touchRes: { isObstacle: boolean, touchSceneObjects: ProjectClientSceneObject[] }): boolean {
        // 恢复存档时当前不会触发事件
        ...
        var subtractList: ProjectClientSceneObject[] = ArrayUtils.compare(touchRes.touchSceneObjects, this.myLastTouchObjects).subtract;
    ...
    protected parallelEventUpdate() {
        ...
        // 未在场景上或未能真正场景完成时忽略执行
        if (!this.inScene || GameGate.gateState < GameGate.STATE_4_PLAYER_CONTROL_START) return;

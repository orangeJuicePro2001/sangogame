...
class GUI_Shop extends GUI_12 {
    ...
    public initGoodsList() {
        ...
        for (let i = 0; i < dataArr.length; i++) {
            ...
            let item: Module_Item | Module_Equip = GameData.getModuleData(moduleID, gooldsID);
            // 道具不存在的情况：忽略
            if (!item) continue;
            data.data = goods;
            data.icon = item.icon;
    ...
    private refreshItemInfo(list: UIList = this.goodsList) {
        ...
        // 已选中道具的情况：显示该道具详情
            ...
            if (this.typeTab.selectedIndex == 0) {
                ...
                var item: Module_Item | Module_Equip = itemDS.isEquip ? GameData.getModuleData(4, itemDS.equip) : GameData.getModuleData(5, itemDS.item);
            }
            else {
                let inPackageItemDS: DataStructure_packageItem = selectedItem.data;
                isEquip = inPackageItemDS.isEquip;
                item = (inPackageItemDS.isEquip ? inPackageItemDS.equip : inPackageItemDS.item) as any;
            }
            if (!item) {
                this.clearItemInfo();
                return;
            ...
            if (isEquip) {
                this.itemIntro.text = GUI_Manager.equipDesc(item as any);
            }
            else {
                this.itemIntro.text = GUI_Manager.itemDesc(item as any);
            }
     ...
     * 刷新持有数量
     */
    private refreshItemInPackage(itemID: number, isEquip: boolean, selectedItem: ListItem_1003) {
        let itemInPackage = ProjectPlayer.getItemDS(itemID, isEquip);
    ...
    public refreshSellItemList() {
        ...
        for (var i = 0; i < Game.player.data.package.length; i++) {
            ...
            if (itemDS.isEquip) {
                // 不允许出售的情况：不列举
                if (!itemDS.equip.sellEnabled) continue;
            ...
            // 绑定数据
            if (itemDS.isEquip) {
                d.icon = itemDS.equip.icon;                   // 图标
                d.itemName = itemDS.equip.name;               // 名称
            }
            else {
